...
United is not asking anyone to rebuild the seat-assignment engine — that exists, and it will keep proposing rebookings the way optimizers do. What's missing is the judgment layer around it: a system that reads each disrupted passenger's full context — the booking record (who's traveling together, connections, special service requests, the free-text remarks agents have added), the proposed rebooking, and the DOT rules that determine what each passenger is legally owed — and decides whether the machine's proposal is acceptable for *this* passenger, fixes or escalates the ones that aren't, and tells every passenger what changed, what they're owed, and what their options are.

### Why a single AI call won't solve this

Judging a proposed rebooking requires reading the itinerary the way an experienced agent does: noticing that the party of four got split, that the new routing connects through a country where this passenger likely needs a transit visa, that the special service request means a tight connection won't work. That's per-passenger reasoning over semi-structured text, and it's different for every record. Determining what the passenger is owed is rule application — the DOT requirements are written down, and the system has to apply them to this passenger's specific situation, not paraphrase them. Communicating the outcome requires generating a message specific to that passenger — what changed, why, what they're entitled to — in a tone that doesn't read like a form letter at the worst moment of their day. Deciding which cases go to a human agent (the unaccompanied minor, the three-leg international itinerary, the group of twelve) is triage, and the escalation is only useful if it arrives with the passenger's situation already assembled into a brief. And the disruption keeps moving — assessments made at 6 PM are stale by 6:40 when two more flights cancel, so the loop has to re-run without churning passengers who were already handled and notified.
...
Detection is genuinely hard, because every individual signal is ambiguous. A shop fulfilling 300 orders a week of "hand-knitted" sweaters is suspicious — unless it's a maker legitimately using a production partner, which Etsy's rules allow with disclosure. Low prices might mean mass production — or a new maker underpricing to get established. A listing description that reads like a wholesale catalog might be copied from one — or written by a maker whose first language isn't English. The signals that separate these cases are mostly text scattered across different places: listing titles and descriptions, the shop's own "about" story and production disclosures, price points implausible for the labor involved, order velocity, buyer reviews ("arrived in AliExpress packaging, took three weeks"), and the output of Etsy's existing image-match service, which flags listings whose photos also appear on wholesale sites — a noisy feed of leads, not verdicts.
...
No single signal is conclusive, so each case requires an investigation loop: read the listing text, check the image-match feed, weigh shop velocity against the plausible output of the claimed production method, read the review text, check whether a production partner is disclosed — and synthesize all of it into a judgment with the evidence cited. Enforcement isn't binary — it's graduated (request production disclosure, restrict the listing, delist, suspend the shop), and each step should require a different evidence bar. Sellers can appeal with evidence of their own — process photos, material receipts — which the system has to actually evaluate rather than rubber-stamp. And enforced resellers re-register under new names, so the system has to recognize returning offenders without flagging every new shop that happens to sell in the same category.
...
Reading tax returns, P&Ls, and bank statements and pulling out the relevant figures is exactly what an LLM can do — but a misread digit in an income figure isn't a typo, it's a wrong loan decision, so every extracted number needs verification: cross-checks against other documents, totals that must reconcile, a citation to the page it came from. Cross-document reconciliation (do the bank deposits support the P&L?) is a different task from extraction. The qualifying income calculation must follow agency guidelines exactly — that's simple deterministic arithmetic with an audit trail, never a language model's mental math. Judging income stability and trend is where reasoning genuinely belongs. The document loop is conditional: finding a Schedule E triggers a need for documents the original checklist never mentioned. And the final decision must stay human — not as a courtesy, but because US lending law requires specific, accurate reasons for every adverse action.
